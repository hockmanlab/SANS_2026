...
- [3 Building the cell-cell neighbourhood
  graph](#3-building-the-cell-cell-neighbourhood-graph)
- [4 Finding clusters](#4-finding-clusters)
  - [4.0.1 Important](#401-important)
  - [4.1 Initial clustering](#41-initial-clustering)
- [5 Testing different clustering
  resolutions](#5-testing-different-clustering-resolutions)
- [6 Computing UMAP](#6-computing-umap)
- [7 Comparing clustering
  resolutions](#7-comparing-clustering-resolutions)
- [8 Selecting a clustering resolution for downstream
  analysis](#8-selecting-a-clustering-resolution-for-downstream-analysis)
- [9 Finding cluster marker genes](#9-finding-cluster-marker-genes)
  - [9.0.1 Important parameters](#901-important-parameters)
  - [9.1 Filtering marker genes](#91-filtering-marker-genes)
- [10 Visualising cluster marker
  genes](#10-visualising-cluster-marker-genes)
- [11 Cluster annotation](#11-cluster-annotation)
  - [11.1 Manual cluster annotation](#111-manual-cluster-annotation)
    - [11.1.1 Important](#1111-important)
- [12 Assigning biological labels](#12-assigning-biological-labels)
- [13 Automated cluster annotation](#13-automated-cluster-annotation)
  - [13.1 Visualise broad predicted cell
    classes](#131-visualise-broad-predicted-cell-classes)
  - [13.2 Visualise predicted
    subclasses](#132-visualise-predicted-subclasses)
- [14 Automated versus manual
  annotation](#14-automated-versus-manual-annotation)
- [15 Summary](#15-summary)
- [16 Further downstream analysis](#16-further-downstream-analysis)
...
# 3 Building the cell-cell neighbourhood graph
...
# 5 Testing different clustering resolutions
...
```
    ...
    ## 10:05:32 UMAP embedding parameters a = 0.9922 b = 1.112

    ## 10:05:32 Read 8598 rows and found 15 numeric columns

    ## 10:05:32 Using Annoy for neighbor search, n_neighbors = 30

    ## 10:05:32 Building Annoy index with metric = cosine, n_trees = 50
    ...
    ## **************************************************|
    ## 10:05:32 Writing NN index file to temp file /var/folders/2r/vkn2d0r15vvbnrb2ybwny3vr0000gn/T//Rtmp2xUeuZ/file754e6b311c64
    ## 10:05:32 Searching Annoy index using 1 thread, search_k = 3000
    ## 10:05:33 Annoy recall = 100%
    ## 10:05:33 Commencing smooth kNN distance calibration using 1 thread with target n_neighbors = 30
    ## 10:05:34 Initializing from normalized Laplacian + noise (using RSpectra)
    ## 10:05:34 Commencing optimization for 500 epochs, with 365058 positive edges
    ## 10:05:34 Using rng type: pcg
    ## 10:05:39 Optimization finished
...
# 7 Comparing clustering resolutions
...
# 8 Selecting a clustering resolution for downstream analysis
...
```
    ...
    ## 1459 1271 1009  788  702  621  515  498  395  261  254  250  206  192  177
...
# 9 Finding cluster marker genes
...
```
    ...
    ##  1 0               3.00 0.643 0.119 0         1       Xirp2   
    ##  2 0               2.92 0.596 0.111 0         1       Il1rapl2
    ##  3 2.55e-254       2.31 0.454 0.098 6.11e-250 1       Gfra2   
    ##  4 8.03e-266       2.29 0.488 0.112 1.92e-261 1       Scube1  
    ##  5 1.35e- 92       2.24 0.255 0.077 3.23e- 88 1       Pld5    
    ##  6 0               4.69 0.275 0.012 0         2       Otof    
    ##  7 2.29e-224       2.94 0.315 0.045 5.48e-220 2       Plk5    
    ##  8 8.22e-163       2.67 0.253 0.04  1.97e-158 2       Glra3   
    ##  9 2.19e-187       2.41 0.334 0.064 5.24e-183 2       Acvr1c  
    ## 10 0               2.40 0.692 0.16  0         2       Gsg1l   
    ## # ℹ 65 more rows
...
# 10 Visualising cluster marker genes
...
```
    ...
    ## SCT assay: Emid1, Nmbr, Carmn, Tbx3os1, Rab37, Cdhr1, Mag, Sox10, Fezf2,
    ## Acvr1c, Glra3
...
## 11.1 Manual cluster annotation
...
# 12 Assigning biological labels
...
# 13 Automated cluster annotation
...
```
    ...
    ## 10:07:56 Read 8598 rows

    ## 10:07:56 Processing block 1 of 1

    ## 10:07:56 Commencing smooth kNN distance calibration using 1 thread with target n_neighbors = 20
    ## 10:07:56 Initializing by weighted average of neighbor coordinates using 1 thread
    ## 10:07:56 Commencing optimization for 67 epochs, with 171960 positive edges
    ## 10:07:56 Finished
    ...
    ## Total elapsed time: 2.09315299987793
...
## 13.1 Visualise broad predicted cell classes
...
## 13.2 Visualise predicted subclasses
...
# 14 Automated versus manual annotation
...
# 16 Further downstream analysis

The dataset used in this workshop does not contain experimental
conditions for performing condition-based differential expression
analysis. However, the same Seurat workflow can be extended to datasets
containing multiple experimental conditions or biological groups.

The [Seurat
documentation](https://satijalab.org/seurat/articles/get_started_v5_new)
provides examples of downstream analyses, including differential
expression and other approaches for exploring single-cell datasets.

For a practical example of differential expression, see the [Seurat
differential expression
vignette](https://satijalab.org/seurat/articles/de_vignette), which uses
the interferon-beta stimulated human PBMCs dataset (ifnb) containing
stimulated and control PBMCs.

These tutorials provide a useful starting point for exploring downstream
analyses beyond the scope of this workshop.
